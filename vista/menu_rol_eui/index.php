    ...
    <div id="tienda_bg" class="container-fluid p-5 my-1">
        <div class="row">
            <div class="col-sm-12">
                ...
                <h2><img src=".\..\img\vinyl_transp.png" style="width:auto;height:80px"> Asignacion de Roles a Items del Menu</h2>
                <p></p>
                <br>
                <table id="dg" style="width:100%;height:450px" url="listar.php" title="Asignar Roles" singleselect="true" fitcolumns="true" toolbar="#toolbar" pagination="true" rownumbers="true">
                    <thead>
                        <tr>
                            ...
                            <th field="descripcion" width="20" sortable="true">Descripcion</th>
...
<script type="text/javascript">
        ...
        if (row) {
            $.messager.confirm('Confirm', 'Seguro que desea asignar este rol?', function(r) {
                if (r) {
                    ...
                    $.post('./accion.php?accion=nuevo_rol&idrol=' + idrol + '&idmenu=' + row.idmenu, {
                            idmenu: row.id
                        },
                        function(result) {
    ...
    function deleterol(idmenu, idrol) {
        
        $.messager.confirm('Confirm', 'Seguro que desea elminar este rol?', function(r) {
            if (r) {
                $.post('./accion.php?accion=borrar_rol&idrol=' + idrol + '&idmenu=' + idmenu, '',
                    function(result) {
                        //alert("Volvio Servidor");
                        if (result.respuesta) {
                            $.messager.show({
    ...
    $(function() {
        $('#dg').datagrid({
            ...
            onExpandRow: function(index, row) {
                var idmenu = row.idmenu;
                var ddv = $(this).datagrid('getRowDetail', index).find('table.ddv');

                ddv.datagrid({
                    url: 'listar_detalle.php?idmenu=' + row.idmenu,
                    fitColumns: true,
                    singleSelect: true,
                    rownumbers: true,
                    loadMsg: '',
                        ...
                        [{
                            ...
                            },
                            {
                                ...
                                formatter: function(value, row, index) {
                                    var c = '<button type="button" class="btn btn-outline-warning btn-sm" onclick="deleterol(' + idmenu + ',' + row.idrol + ')">Eliminar</button>';
                                    return c;
                                }
                            }
                        ]
